...
hold on
semilogx(wall_normal_coors,log(wall_normal_coors)/0.41+7.5)
hold on
linear_ref = 1:0.05:32;
semilogx(linear_ref,linear_ref)
hold on
semilogx(linear_ref,log(linear_ref)/0.41+7.5)
hold on


matching_loc = 1:1:32;
wall_parallel_vel = U;%-correction_para;
wall_parallel_vel = wall_parallel_vel(:,:,2:end);
tic
for i = 1:length(matching_loc)
    res = extract_2d_slice_x_interp(wall_parallel_vel, wall_normal_coors, matching_loc(i)/3200,re);
    a(i) = mean(res(:));
end
toc
